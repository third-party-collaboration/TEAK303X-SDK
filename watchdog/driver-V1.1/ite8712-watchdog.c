/*
 $Id: oak2030_watchdog.c 2007-02-13 10:56:59 mark Hsieh $

 Linux kernel watchdog interface compatible wachdog driver for Arinfotek NSM5010

 This code is a watchdog device driver used to control the ITE-8712 Super IO as watchdog
 timer function.
 The code works in Pentium-M or Celeron-M processor with south bridge ICH6.
 Application uses this driver to reset the timer by invoking the function in watchdog device
 to avoid the system reset.

 All rights are reserved.
 Mark Hsieh V1.00  2007/02/13 -- initial version
 Mark Hsieh V1.10  2009/05/13 -- Update code to meet the OAK2030 board
*/
...
struct oak2030_watchdog_struct {
	int 		status;			// status
...
#define LOCAL_VERSTRING 	"oak2030: 1.00"
static char *watchdog_version = "$Rev: 1.00$";
static char *watchdog_revdate = "$Date: 2009-05-13 10:56:59 +0100 (Tues., 13 May. 2009) $";

struct oak2030_watchdog_struct oak2030_watchdog_info;
...
// set enter key
	...
	oak2030_watchdog_info.gamePort_base = gamePort;
	gamePort= watchdog_getValue(0x61);
	oak2030_watchdog_info.gamePort_base <<= 8;
	oak2030_watchdog_info.gamePort_base |= gamePort;
...
static void reset_watchdog(void)
{
	...
	// printk("oak2030_watchdog: reset timer base=0x%X\n",oak2030_watchdog_info.gamePort_base);
	outb(0x01, oak2030_watchdog_info.gamePort_base);
}
			    ...
			    unsigned int cmd, unsigned long arg)
{
	...
	case WDIOC_GETBOOTSTATUS:
		if (put_user(oak2030_watchdog_info.status, (int *)arg))
			return -EFAULT;
		oak2030_watchdog_info.status &= ~WDIOF_KEEPALIVEPING;
		//printk("watchdog: status=%X\n", oak2030_watchdog_info.status);
		break;
	...
	case WDIOC_SETTIMEOUT:
		...
		oak2030_watchdog_info.timeout_counter = setCounter;
	...
	case WDIOC_GETTIMEOUT:
		if (put_user(oak2030_watchdog_info.timeout_counter, (u16 *)arg))
			return -EFAULT;
		break;
	default:
		return -ENOIOCTLCMD;
...
static int watchdog_open(struct inode *inode,  struct file * filp)
{
	...
	if (test_and_set_bit(0, &oak2030_watchdog_info.opened)) return -EBUSY;
	//printk("oak2030_watchdog: open device entry\n");
	// set enter key
	watchdog_enterKey();
   ...
   // id2= watchdog_getValue(0x21);
	// printk("oak2030_watchdog: ID1=0x%X  ID2=0x%X\n", id1, id2);
	...
	// LSB Register
	// MSB Register
	set= (unsigned char) (oak2030_watchdog_info.timeout_counter & 0xFF);
	watchdog_setValue(WDC_TIMEOUT_LSB, set);

	// MSB Register
	set= (unsigned char) ((oak2030_watchdog_info.timeout_counter >> 8) & 0xFF);
	watchdog_setValue(WDC_TIMEOUT_MSB, set);
	...
	oak2030_watchdog_info.gamePort_base <<= 8;
	oak2030_watchdog_info.gamePort_base |= gamePort;
	printk("oak2030_watchdog: Rd game port base= 0x%X\n", oak2030_watchdog_info.gamePort_base);
	if(oak2030_watchdog_info.gamePort_base == 0) {
		watchdog_setValue(0x60, 0x02);   // high value
		watchdog_setValue(0x61, 0x01);	// low value
		oak2030_watchdog_info.gamePort_base= 0x201;		
		//printk("oak2030_watchdog: Reset game port base= 0x%X\n", oak2030_watchdog_info.gamePort_base);
    ...
    // bit-1
    watchdog_setValue(WDC_EXIT_MODE, 0x02);
	//printk("oak2030_watchdog: init device exit\n");
...
static int watchdog_release(struct inode *inode, struct file * filp)
{
	...
	printk("oak2030_watchdog: device closed \n");
	clear_bit(0, &oak2030_watchdog_info.opened);
...
{
	printk("oak2030_watchdog: version %s (%s)\n",
	       watchdog_version, watchdog_revdate);
	return;
}

static struct file_operations oak2030_watchdog_fops = {
	.owner = THIS_MODULE,
...
static struct miscdevice oak2030_watchdog_miscdev = {
	.minor = WATCHDOG_MINOR,
	.name = "watchdog",
	.fops = &oak2030_watchdog_fops,
};


static int __init oak2030_watchdog_init(void)
{
	int result;
   struct pci_dev *dev = NULL;
	...
	oak2030_watchdog_info.opened = 0;
	oak2030_watchdog_info.timeout_counter = 0x0100;

	printk("oak2030_watchdog: module driver installed\n");
   while ((dev = pci_find_device(PCI_ANY_ID, PCI_ANY_ID, dev)) != NULL) {
	   if(dev->vendor == PCI_VENDOR_ID_INTEL) {
		   if((dev->device == PCI_DEVICE_ID_INTEL_ICH7_1)||
		     (dev->device == PCI_DEVICE_ID_INTEL_ICH7_0)){
	...
	result = misc_register(&oak2030_watchdog_miscdev);
	if (result < 0) {
		printk("oak2030_watchdog: Couldn't register watchdog driver\n");
		return result;
	}
...
static void __exit oak2030_watchdog_fini(void)
{
	int e1;
	
	e1 = misc_deregister(&oak2030_watchdog_miscdev);

	if (e1 < 0) printk("oak2030_watchdog: failed to unregister watchdog driver (%d)\n",  e1);
}

module_init(oak2030_watchdog_init);
module_exit(oak2030_watchdog_fini);
MODULE_DESCRIPTION("OAK2030 (ITE8712 Super IO) watchdog driver");
MODULE_LICENSE("GPL");
