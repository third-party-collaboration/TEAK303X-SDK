/* lcm-net.c - A lcm protocol program is to communicate with the lcm controller running 8051 via serial port */
      ...
      if(ret > 0) {
         ...
         case 'L':
             lcm_move_line2();
               break;
		 case 's':
			 lcm_print_msg1(testBuf);
			  break;
		 case 't':
			 lcm_print_msg2(testBuf);
			  break;
         case '1':
...
//              lcm_print_msg1((unsigned char *)"1-12345678");
              break;
         case '2':
               lcm_print_msg2((unsigned char *)"2-1234567890abcdefgh");
              break;
         case 'b':
